...
import com.product.managefund.databinding.FragmentDanaKelolaanBinding
...
class DanaKelolaanFragment : Fragment() {
    ...
    private lateinit var binding: FragmentDanaKelolaanBinding
    ...
    override fun onCreateView(inflater: LayoutInflater,container: ViewGroup?,savedInstanceState: Bundle?): View? {
        ...
        binding = FragmentDanaKelolaanBinding.inflate(inflater, container, false)
    ...
    fun setChartFilter(filter : Int){
        ...
        if(filter == 0){
            ...
            for(i in listOfGraphOne.size downTo listOfGraphOne.size - 5 ){
                firstChart?.add(listOfGraphOne[i-1].value.toFloat())
                firstIndex++
            }
            ...
            for(j in listOfGraphTwo.size downTo listOfGraphTwo.size - 5){
                secondChart?.add(listOfGraphTwo[j-1].value.toFloat())
                firstIndex++
            }
            ...
            if(listOfProduct.size > 2){
                for(j in listOfGraphThree.size downTo listOfGraphThree.size - 5){
                    thirdChart?.add(listOfGraphThree[j-1].value.toFloat())
                    firstIndex++
                }
            }
